...
void VCR_DrawEffect(int screen_width, int screen_height, float time)
{
    ...
    /* 4. Tracking Line (if active) - 2 LINES as requested */
    if (show_tracking && CVAR_INT(vcr_tracking_lines)) {
        ...
        /* Line 2 (Offset by ~150px up) */
        float y2 = y - 150.0f;
        vcr_draw_rect(0, y2, (float)vcr.width, h, 0.1f, 0.1f, 0.1f, 0.3f);
        vcr_draw_rect(0, y2 - 1, (float)vcr.width, 1, 1.0f, 0.0f, 0.0f, 0.1f);
        vcr_draw_rect(0, y2 + h, (float)vcr.width, 1, 0.0f, 1.0f, 1.0f, 0.1f);
    }
